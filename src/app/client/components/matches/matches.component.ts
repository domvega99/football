              ...
              this.leagueMatches = leagueRes.map((league: any) => {
                return {
                  ...
                  matches: league.matches.map((matchDay: any) => {
                    let leagueMatchesToDisplay = matchDay.matches.map((match: any) => ({
                      ...match,
                    ...
                    const filteredCombinedMatches = combinedMatches.filter((match: any) =>
                      match.status === 'Posted' || match.status === 'Live'
                    );
  
                    return {
                      ...matchDay,
                      matches: filteredCombinedMatches 
                    };
              ...
              this.leagueMatches.forEach(league => {
                league.matches.forEach((matchDay: any) => {
                  if (totalMatchesToDisplay > 0) {
                    ...
                    matchDay.matches = matchesToDisplay;
                    totalMatchesToDisplay -= matchesToDisplay.length;
                  } else {
                    matchDay.matches = [];
                  }
                });
